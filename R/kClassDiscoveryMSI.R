...
kClassDiscoveryMSI <- function(path,
  ...
  for (i in 1:length(feature_Nr)) {
    ...
    if(is.null(ZeroAction)) {
      ...
      if(length(which(matrixStats::colSds(runner) == 0)) > 0) {
      ...
      runner_WO_zeros <- as.matrix(runner[,as.numeric(matrixStats::colSds(runner)) != 0])

      runner_coords_WO_zeros <- as.matrix(runner_coords[,as.numeric(matrixStats::colSds(runner)) != 0])
    ...
    if(show_condition(pvclust::pvclust(runner_WO_zeros, nboot = 10,
      ...
      HCA_boot_feature <- pvclust::pvclust(runner_WO_zeros,
      ...
      if (dim(runner_WO_zeros)[2] > 0.2 * dim(runner)[2]) {
        ...
        table_features <- pvclust::pvpick(HCA_boot_feature, alpha = alpha, pv="au")
        ...
        if (Initial_k < length(table_features$edges)) {
          ...
          table_features <- pvclust::pvpick(HCA_boot_feature, alpha = alpha_out, pv="au")
