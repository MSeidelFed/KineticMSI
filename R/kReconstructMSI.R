...
kReconstructMSI <- function(Reconstruct = c("After", "Before"),
                            kClustersMSI,
                            EnrPath,
                            GetCoords = c("Cardinal", "SCiLS"),
                            CoordsFilePath = system.file("extdata", package = "KineticMSI"),
  ...
  ###### Coord Files & compatibility with Enrichment Files
  
  if (GetCoords == "SCiLS") {
    
    ##### SCiLS files
    
    MSI_files <- list.files(path = CoordsFilePath, pattern = ".csv",
                            all.files = FALSE, full.names = TRUE, recursive = TRUE,
                            ignore.case = FALSE, include.dirs = TRUE, no.. = FALSE)
    
    
  } else if (GetCoords == "Cardinal") {
    
    ##### MSI files
    
    MSI_files <- list.files(path = CoordsFilePath, pattern = ".imzML",
                            all.files = FALSE, full.names = TRUE, recursive = TRUE,
                            ignore.case = FALSE, include.dirs = TRUE, no.. = FALSE)
  ...
  for (m in 1:length(Enrichment_files)) {
    ...
    if (GetCoords == "SCiLS") {
      ...
      coords_file <- read.table(file = MSI_files[m], header = T, sep = ",") 
      ...
      if (as == "MSImageSet") {
        ...
        stop("If using Cardinal, only MSImagingExperiment or MSImageSet classes are supported")
        
      }
      
    } else {
      
      stop("Only Cardinal or SCiLS can be used to create coordinates")
