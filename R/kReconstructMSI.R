...
#' @param path parent directory were all MSI and enrichment files are contained, digs within recursive folders.
...
kReconstructMSI <- function(Reconstruct = c("After", "Before"),
  ...
  Enrichment_files <- list.files(path = path, pattern = PatternEnrichment,
  ...
  MSI_files <- list.files(path = path, pattern = ".imzML",
                          all.files = FALSE, full.names = TRUE, recursive = TRUE,
                          ignore.case = FALSE, include.dirs = TRUE, no.. = FALSE)
  ...
  names(cluster_list_of_lists) <- MSI_files
  
  if(returnObject == T) {
    
    return(cluster_list_of_lists)
    
    cat("Finished building the output plots and return lists of clusters")
