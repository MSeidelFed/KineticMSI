#' A function to perform an initial step of replicated data quality assessment on KineticMSI datasets
#'
...
#' @param returnObject can be either "RowMeansDataset" or "minDataset" and returns either a list with input matrices per molecular feature for the following mean class comparison (only the rowmeans per feature) or the minimum data subset to the minimum amount of pixels per entity.
...
kAssesmentMSI <- function(path,
  ...
  if (CompareSampledSet == T) {
    ...
    for (i in 1:length(reps)) {
      ...
      CompleteSet_SD <- c(CompleteSet_SD, matrixStats::rowSds(as.matrix(runner_file)))
    ...
    MinSet_SD <- matrixStats::rowSds(out_mat)
  ...
  for (i in 1:length(feature_Nr)) {
    ...
    cat(paste0("Mean: ",
               round(mean(runner[[1]]), digits = 3),
               "; St.Dev: ",
               round(sd(runner[[1]]), digits = 3), "\n"))
    cat("...\n")
    ...
    #### generating the second output (Rowmeans input & distribution test)
    ...
    if (dim(runner_WO_zeros)[2] < 1) {
      ...
      plot_name <- unique(RandoDiStats::testing_distributions(Distribution_test_mat = cbind(as.numeric(out_mat2[,1]),
  ...
  } else if(is.null(returnObject) | returnObject == "RowMeansDataset") {
