...
kSummaryMSI <- function(kComparisonOutput,
  ...
  FisherExactMSI <- function(FisherDir,
    ...
    for (i in 1:dim(Fisher_terms)[2]) {
      ...
      for (j in 1:length(Categories)) {
        ...
        names_mats[j] <- paste0(Enrichment_Term,";", Categories[j], ";",
                                percent(round(pSigEntInClassToTotEntInClass, 2)),
                                ";", percent(round(pEntInClassToTotEnt, 2)),
                                ";", SigEntInCat)
    ...
    in_object <- read_csv("FisherResults.csv")
    ...
    simplePlot <- ggplot(in_object, aes(x = -log10(Fisher_P_value),
                                        y = fct_reorder((Assigned_function),
                                                        as.numeric(as.factor(Functional_Categories))),
                                        fill = Functional_Categories)) +
      geom_col(width=0.9) +
      labs(x = expressionXaxis, y = NULL) +
      geom_vline(xintercept = -log10(0.05), color='red', linetype = "solid")+
      geom_text(aes(label = labels), hjust = -0.3, color="black", size = FisherCatNumSizes)+
      scale_fill_viridis_d(option = "inferno")+
      theme_classic()+
      xlim(LowerLimitsFisher, UpperLimitsFisher)+
      labs(fill = "Functional Categories")+
      theme(axis.text.y = element_text(size = FisherCatAxisFontSizes),
      ...
      theme(legend.position=FisherLegendPosition)+
      guides(color = guide_legend(override.aes = list(size= FisherLegendKeySizes)))
  ...
  for (i in 1:length(list_SubSets)) {
    ...
    if(class(list_SubSets[[i]]) == "matrix") {
      ...
      plot_list[[i]] <- ggplot(data, aes(x = Abscissas, y = Ordinates, color = factor(V3))) +
        geom_point(shape = Colors_shape_corrected, size = DotsSize) +
        scale_color_manual(values = Legend_input[,2], breaks = Legend_input[,1]) + #  , "-" = "#e5e5e5"
        labs(color = ColorCategory, x = AbscissasName, y = OrdinatesName) +
        theme_classic(base_size = 12) +
        theme(axis.text = element_text(size=AxesIndexSize),
              axis.title = element_text(size=AxesTitleSize, face="bold"),
              legend.text = element_text(size= LegendFontSize),
              legend.title = element_text(size=LegendTitleSize, face="bold"),
              legend.position = VolcanoLegendPosition) +
        geom_hline(yintercept = c(-log10(0.05), -log10(SigBndrie)),
                   linetype=c("dashed", "solid"),
                   color = c("orange", "red")) +
        geom_vline(xintercept = c(-xBndrie,-0.2,0.2,xBndrie),
                   linetype=c("solid","dashed","dashed", "solid"),
                   color = c("red","orange","orange", "red")) +
        ggtitle(names(list_SubSets)[i]) +
        geom_text_repel(label = Labels_corrected, show.legend = F, size = LabelSize) +
        xlim(LowerLimitVolcano, UpperLimitVolcano) +
        guides(color = guide_legend(override.aes = list(size=LegendKeySize)))
    ...
    if (returnPlotsPNGs == T) {
      ...
      ggsave(plot_list[[i]], file=paste0("plot_", names(list_SubSets)[i], "_", plotTitle,".png"),
