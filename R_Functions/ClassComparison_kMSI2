...
ClassComparison_kMSI2 <- function(ClassDiscoveryList,
                                  factorVector,
      ...
      if (ClassComparison == FALSE) {
        ...
        if (length(unique(melt(mat_to_melt)$Var2)) < 2) {
          ...
          for (i in 1:length(P_values)) {
            ...
            } else if (P_values[i] < 0.1 & P_values[i] > 0.05) {
              runner = "*"
            } else if (P_values[i] < 0.05 & P_values[i] > 0.01) {
              runner = "**"
            } else if (P_values[i] < 0.01) {
          ...
          P_value[[m]] <- TUKEY$`data$treatment`[,"p adj"]
